"""
Sales Forecasting Dashboard - Q2 2026 Version
Reads from Google Sheets and displays gap-to-goal analysis with interactive visualizations
Includes lead time logic for Q2/Q3 fulfillment determination and detailed order drill-downs
...
# Custom CSS — minimal, works with Streamlit light theme from config.toml
st.markdown("""
    <style>
...
# These functions handle both old ('Q2 2026 Spillover') and new ('Q3 2026 Spillover') column names
...
def load_all_data():
    ...
    # Q4 Push planning status removed for Q2 dashboard
    ...
    # - Date range: Q2 2026 (Apr 1 - Jun 30, 2026)
    ...
    if not deals_df.empty and len(deals_df.columns) >= 6:
        ...
        for col in col_names:
            ...
            if 'Deal Owner First Name' in col and 'Deal Owner Last Name' in col:
                ...
                rename_dict[col] = 'Q3 2026 Spillover'
    ...
    if not invoices_df.empty:
        if len(invoices_df.columns) >= 15:
            ...
            # Filter to Q2 2026 only (4/1/2026 - 6/30/2026)
            ...
            invoices_df = invoices_df[
                (invoices_df['Date'] >= q2_start) & 
                (invoices_df['Date'] <= q2_end)
            ]
...
def store_snapshot(deals_df, dashboard_df, invoices_df, sales_orders_df, q4_push_df=None):
    ...
    if 'q2_previous_snapshot' not in st.session_state:
...
def create_dod_audit_section(deals_df, dashboard_df, invoices_df, sales_orders_df):
    ...
    if 'q2_previous_snapshot' in st.session_state and st.session_state.previous_snapshot:
        ...
        with hs2_col2:
            current_q2 = current_metrics.get('q2_spillover_expect_commit', 0)  # Key mapped to Q2
            previous_q2 = previous_metrics.get('q2_spillover_expect_commit', 0)
...
def build_your_own_forecast_section(metrics, quota, rep_name=None, deals_df=None, invoices_df=None, sales_orders_df=None, q4_push_df=None, production_schedule_df=None, amie_update_df=None):
    ...
    # Planning status feature removed for Q2 dashboard
    # (The Q1 Push sheet is not used in Q2)
    ...
    ns_dfs = {
        ...
        'PF_Q4_Spillover': format_ns_view(so_categories['pf_q1_spillover'], 'Promise'),
        'PF_Q2_Spillover': format_ns_view(so_categories['pf_q3_spillover'], 'Promise'),
    ...
    if not hs_data.empty:
        ...
        if spillover_col == 'Q3 2026 Spillover':
            ...
            q1 = (spillover_vals != 'Q3 2026') & (spillover_vals != 'Q1 2026')
            q4 = spillover_vals == 'Q1 2026'
            q2 = spillover_vals == 'Q3 2026'
        elif spillover_col == 'Q2 2026 Spillover':
            # Old column name - for Q2 dashboard, all deals are primary quarter
            # The old column was marking deals that spill TO Q2, but now we're IN Q2
        ...
        hs_dfs['Q1_Expect'] = format_hs_view(hs_data[q4 & (hs_data['Status'] == 'Expect')])
        hs_dfs['Q1_Commit'] = format_hs_view(hs_data[q4 & (hs_data['Status'] == 'Commit')])
        hs_dfs['Q1_BestCase'] = format_hs_view(hs_data[q4 & (hs_data['Status'] == 'Best Case')])
        hs_dfs['Q1_Opp'] = format_hs_view(hs_data[q4 & (hs_data['Status'] == 'Opportunity')])
        hs_dfs['Q3_Expect'] = format_hs_view(hs_data[q2 & (hs_data['Status'] == 'Expect')])
        hs_dfs['Q3_Commit'] = format_hs_view(hs_data[q2 & (hs_data['Status'] == 'Commit')])
        hs_dfs['Q3_BestCase'] = format_hs_view(hs_data[q2 & (hs_data['Status'] == 'Best Case')])
        hs_dfs['Q3_Opp'] = format_hs_view(hs_data[q2 & (hs_data['Status'] == 'Opportunity')])

    # --- 4. RENDER UI & CAPTURE SELECTIONS ---
    # Uses st.form to batch edits — NO reruns until user clicks Apply.
    # Category filtering via checkboxes with full visible names.
    ...
    for key, data in ns_categories.items():
        cat_df = ns_dfs.get(key, pd.DataFrame())
        if not cat_df.empty and 'Amount_Numeric' in cat_df.columns:
            val = cat_df['Amount_Numeric'].sum()
            if val > 0 or key == 'PA_Date':
                rows = cat_df.copy()
                rows['Category'] = data['label']
                rows['_cat_key'] = key
                ns_combined_rows.append(rows)
                seen_cat_keys.add(key)
                ns_cat_totals[key] = (data['label'], val, len(rows))
    if 'PA_Date' not in seen_cat_keys:
        pa_df = ns_dfs.get('PA_Date', pd.DataFrame())
        if not pa_df.empty:
            rows = pa_df.copy()
            rows['Category'] = ns_categories['PA_Date']['label']
            rows['_cat_key'] = 'PA_Date'
            ns_combined_rows.append(rows)
            ns_cat_totals['PA_Date'] = (ns_categories['PA_Date']['label'], 0, len(rows))
    ...
    for key, data in hs_categories.items():
        ...
        if not cat_df.empty and 'Amount_Numeric' in cat_df.columns and cat_df['Amount_Numeric'].sum() > 0:
            rows = cat_df.copy()
            rows['Category'] = data['label']
            rows['_cat_key'] = key
            hs_combined_rows.append(rows)
            hs_cat_totals[key] = (data['label'], cat_df['Amount_Numeric'].sum(), len(rows))

    combined_hs = pd.concat(hs_combined_rows, ignore_index=True) if hs_combined_rows else pd.DataFrame()
    ...
    with ns_tab:
        ...
        if not combined_ns.empty and 'SO #' in combined_ns.columns:
            ...
            # --- Category filter: checkboxes with full names ---
            st.markdown("**Show categories:**")
            ns_filter_cols = st.columns(min(len(ns_cat_totals), 3))
            ns_selected_cats = set()
            for i, (cat_key, (label, total, count)) in enumerate(ns_cat_totals.items()):
                col = ns_filter_cols[i % len(ns_filter_cols)]
                if col.checkbox(f"{label} (${total:,.0f}, {count})", value=True, key=f"ns_cf_{cat_key}_{rep_name}"):
                    ns_selected_cats.add(cat_key)

            working_ns = combined_ns[combined_ns['_cat_key'].isin(ns_selected_cats)].copy() if ns_selected_cats else pd.DataFrame()
            ...
            if working_ns.empty:
                ...
                # Search filter (outside form so it filters immediately)
                ...
                if display_df.empty:
                    ...
                    # === FORM: all edits batched, no reruns until Apply ===
                    with st.form(key=f"ns_form_{rep_name}"):
                        ...
                        ns_submitted = st.form_submit_button("✅ Apply Changes", use_container_width=True, type="primary")
                    ...
                    # Build export_buckets from current state (whether just submitted or from prior state)
                    # Use saved state to determine selections
                    ns_export = {}
                    saved_st = st.session_state[ns_state_key]
                    for cat_key in ns_categories:
                        ...
                        if not cat_df.empty and 'SO #' in cat_df.columns:
                            ...
                            if selected_rows:
                                ns_export[cat_key] = pd.concat(selected_rows)
                    ...
                    # Show totals
                    ns_total = sum(df['Amount_Numeric'].sum() for df in ns_export.values() if not df.empty and 'Amount_Numeric' in df.columns)
                    ns_count = sum(len(df) for df in ns_export.values())
                    st.caption(f"**Selected NetSuite Total: ${ns_total:,.0f}** ({ns_count} orders)")
    ...
    with hs_tab:
        ...
        if not combined_hs.empty and 'Deal ID' in combined_hs.columns:
            ...
            # --- Category filter: checkboxes with full names ---
            st.markdown("**Show categories:**")
            hs_filter_cols = st.columns(min(len(hs_cat_totals), 3))
            hs_selected_cats = set()
            for i, (cat_key, (label, total, count)) in enumerate(hs_cat_totals.items()):
                col = hs_filter_cols[i % len(hs_filter_cols)]
                if col.checkbox(f"{label} (${total:,.0f}, {count})", value=True, key=f"hs_cf_{cat_key}_{rep_name}"):
                    hs_selected_cats.add(cat_key)

            working_hs = combined_hs[combined_hs['_cat_key'].isin(hs_selected_cats)].copy() if hs_selected_cats else pd.DataFrame()
            ...
            if working_hs.empty:
                ...
                if display_hs.empty:
                    ...
                    # === FORM: all edits batched, no reruns until Apply ===
                    with st.form(key=f"hs_form_{rep_name}"):
                        ...
                        hs_submitted = st.form_submit_button("✅ Apply Changes", use_container_width=True, type="primary")
                    ...
                    # Build export_buckets from saved state
                    hs_export = {}
                    saved_hst = st.session_state[hs_state_key]
                        ...
                        if not cat_df.empty and 'Deal ID' in cat_df.columns:
                            ...
                            for cidx in cat_df.index:
                                ...
                                if did_state.get('select', True):
                                    ...
                                    selected_rows.append(row)
                            if selected_rows:
                                hs_export[cat_key] = pd.concat(selected_rows)
                    ...
                    if use_probability:
                        hs_total = sum(df['Prob_Amount_Numeric'].sum() for df in hs_export.values() if not df.empty and 'Prob_Amount_Numeric' in df.columns)
                    else:
                        hs_total = sum(df['Amount_Numeric'].sum() for df in hs_export.values() if not df.empty and 'Amount_Numeric' in df.columns)
                    hs_count = sum(len(df) for df in hs_export.values())
                    st.caption(f"**Selected HubSpot Total: ${hs_total:,.0f}** ({hs_count} deals)")
    ...
    ">
        <div style="display: flex; flex-direction: column; align-items: center; padding: 0 1rem;">
            <div style="font-size: 0.6rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: #94a3b8; margin-bottom: 4px;">INVOICED</div>
            <div style="font-size: 1.15rem; font-weight: 700; color: #059669;">${invoiced_shipped:,.0f}</div>
        </div>
        <div style="width: 1px; height: 36px; background: #e2e8f0;"></div>
        <div style="display: flex; flex-direction: column; align-items: center; padding: 0 1rem;">
            <div style="font-size: 0.6rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: #94a3b8; margin-bottom: 4px;">+ PENDING</div>
            <div style="font-size: 1.15rem; font-weight: 700; color: #d97706;">${selected_pending:,.0f}</div>
        </div>
        <div style="width: 1px; height: 36px; background: #e2e8f0;"></div>
        <div style="display: flex; flex-direction: column; align-items: center; padding: 0 1rem;">
            <div style="font-size: 0.6rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: #94a3b8; margin-bottom: 4px;">{pipeline_label}</div>
            <div style="font-size: 1.15rem; font-weight: 700; color: #4f46e5;">${selected_pipeline:,.0f}</div>
        </div>
        <div style="width: 1px; height: 36px; background: #e2e8f0;"></div>
        <div style="display: flex; flex-direction: column; align-items: center; padding: 0 1rem;">
            <div style="font-size: 0.6rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: #94a3b8; margin-bottom: 4px;">= FORECAST</div>
            <div style="font-size: 1.3rem; font-weight: 800; color: #0f172a;">${total_forecast:,.0f}</div>
        </div>
        <div style="width: 1px; height: 36px; background: #e2e8f0;"></div>
        <div style="display: flex; flex-direction: column; align-items: center; padding: 0 1rem;">
            <div style="font-size: 0.6rem; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: #94a3b8; margin-bottom: 4px;">{gap_label}</div>
            <div style="font-size: 1.15rem; font-weight: 700; color: {gap_color};">{gap_display}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def categorize_sales_orders(sales_orders_df, rep_name=None):
    ...
    empty_result = {
        ...
        'pa_q3_spillover': pd.DataFrame(), 'pa_q3_spillover_amount': 0
    }
    ...
    # PF Spillover: PF with Date orders where ship date > Q2 end
    # Ship date = min(Customer Promise Date, Projected Date) — same as Apps Script
    pf_q3_spillover = pd.DataFrame()
    if not pf_date_ext.empty or not pf_date_int.empty:
        ...
        if not pf_with_date_all.empty:
            ...
            spill_mask = pf_with_date_all['_ship_date'].notna() & (pf_with_date_all['_ship_date'] > q2_end)
            pf_q3_spillover = pf_with_date_all[spill_mask].copy()
            
            # Remove spillover orders from their original buckets
            if not pf_q3_spillover.empty:
                spill_indices = pf_q3_spillover.index if not pf_q3_spillover.empty else pd.Index([])
                ...
                if not pf_date_ext.empty:
                    ...
                    ext_spill = pf_date_ext['_ship_date'].notna() & (pf_date_ext['_ship_date'] > q2_end)
                ...
                if not pf_date_int.empty:
                    ...
                    int_spill = pf_date_int['_ship_date'].notna() & (pf_date_int['_ship_date'] > q2_end)
                    pf_date_int = pf_date_int[~int_spill].copy()
    
    # PA Spillover: PA with Date orders where Pending Approval Date > Q2 end
    pa_q3_spillover = pd.DataFrame()
    if not pa_date.empty:
        pa_date['_pa_dt'] = pd.to_datetime(pa_date['Display_PA_Date'], errors='coerce')
        pa_spill_mask = pa_date['_pa_dt'].notna() & (pa_date['_pa_dt'] > q2_end)
        pa_q3_spillover = pa_date[pa_spill_mask].copy()
        pa_date = pa_date[~pa_spill_mask].copy()
    
    # Q1 spillover — not currently tracked via dates, keep empty
    pf_q1_spillover = pd.DataFrame()
    pa_q1_spillover = pd.DataFrame()
    ...
    return {
        ...
        'pa_q1_spillover_amount': get_amount(pa_q1_spillover),
        'pf_q3_spillover': pf_q3_spillover,
        'pf_q3_spillover_amount': get_amount(pf_q3_spillover),
        'pa_q3_spillover': pa_q3_spillover,
...
def display_team_dashboard(deals_df, dashboard_df, invoices_df, sales_orders_df, q4_push_df=None, production_schedule_df=None, amie_update_df=None):
    ...
    st.title("🎯 Team Sales Dashboard - Q2 2026")
    ...
    team_q2_spillover_expect_commit = 0  # Q1 spillover Expect/Commit only
    team_q2_spillover_best_opp = 0  # Q1 spillover Best Case/Opportunity
    ...
    for rep_name in dashboard_df['Rep Name']:
        ...
        if rep_metrics:
            ...
            team_q2_spillover_expect_commit += rep_metrics.get('q2_spillover_expect_commit', 0)
            team_q2_spillover_best_opp += rep_metrics.get('q2_spillover_best_opp', 0)
    ...
    if not deals_df.empty:
        ...
        deals_q2 = pd.DataFrame()
    team_best_case = deals_q2[deals_q2['Status'] == 'Best Case']['Amount'].sum() if not deals_q2.empty and 'Status' in deals_q2.columns else 0
    ...
    section2_data.append({
        ...
        'Total Q2': f"${full_forecast:,.0f}"
    })
   
    # Display Q2 spillover info if applicable
    team_q3_spillover_total = team_q2_spillover_expect_commit + team_q2_spillover_best_opp
    if team_q3_spillover_total > 0:
        st.info(
            f"ℹ️ **Q3 2026 Spillover**: ${team_q3_spillover_total:,.0f} in deals closing late Q2 2026 "
            f"will ship in Q3 2026 due to product lead times. These are excluded from Q2 revenue recognition."
        )
    ...
    team_metrics_for_forecast = {
        ...
        'q2_spillover_expect_commit': team_q2_spillover_expect_commit,
        'q2_spillover_best_opp': team_q2_spillover_best_opp
    }
...
def main():
    ...
    if view_mode == "Team Overview":
        ...
        if not dashboard_df.empty:
            # FIX: Added key="q2_rep_selector" to preserve selection across refreshes
...
def render_q2_revenue_snapshot(view_mode=None):
